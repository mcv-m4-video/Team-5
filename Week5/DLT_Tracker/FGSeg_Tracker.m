...
seq.s_frames = cell(150, 1);
seq.Tracks = cell(150, 1);

RoI = zeros(size(Im, 1), size(Im, 2));
RoI([1:20,end-20:end],:) = 1;
TCount = 0;


%% Foreground extraction
ForeGroundR = ForegroundExt_Gaussian(Im, BFG);


for i = 1:size(ForeGroundR.ForeG, 3) %1 : size(d, 1)
    
    seq.s_frames{i} = ForeGroundR.ForeG(:,:,i);
    seq.RGB_frames{i} = RGB(:,:,:,i+150);
...
se = strel('square', 7);
Count = 1;
for phi = 0.4 %0:0.01:1
    for Alpha = 13 %[0.1:0.1:0.9,1:15]
        ...
        %         for imgC = 1:size(RIms, 3)
        %             imwrite(RIms(:,:,imgC), ['HIGHWAY\Results_AfMorph\Out_' sprintf('%0.6d', imgC) '.jpg']);
        %         end
