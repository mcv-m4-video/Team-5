...
DatabaseName = 'HIGHWAY';
[RGB,Im, GT, BG, BFG] = ReadingFrames(DatabaseName);
...
[ForeGroundR ] = ForegroundExt_Gaussian(Im, BFG,BestPhi);
...
% 
% BackGroundParts = ForeGroundR(BGPhiAlphaValLoc).ForeG;
% ForeGroundParts = ForeGroundR(PhiAlphaValLoc).ForeG;
% ForeOnly = (ForeGroundParts - BackGroundParts)>0;
% 
% % wrt foreground FO (substracted foreground)
% for i = 1:length(ForeGroundR)
...
Kalman_tracking_MarkIII(RGB(:,:,:,size(RGB,4)/2+1:end), ForeGroundR(GTPhiAlphaValLoc).ForeG);
...
%% Reading frames
function [RGB,Im, GT, BG, BckGForeG] = ReadingFrames(DatabaseName)
...
if(strcmp(DatabaseName, 'TRAFFIC'))
    ...
    for i = 951:1050
        ...
        RGB(:,:,:,i-951+1) = imread(['../../Datasets/traffic/input/in00' sprintf('%0.4d', i) '.jpg']);
        GTimg = imread(['../../Datasets/traffic/groundtruth/gt00' sprintf('%0.4d', i) '.png']);
        GT(:,:,i-951+1) = GTimg == 255;
        BG(:,:,i-951+1) = GTimg == 50; %GTimg == 0 | GTimg == 50;
...
elseif(strcmp(DatabaseName, 'HIGHWAY'))
    ...
    for i = 1051:1350
        ...
        RGB(:,:,:,i-1051+1) = imread(['../../Datasets/highway/input/in00' sprintf('%0.4d', i) '.jpg']);
        Gtimg = imread(['../../Datasets/highway/groundtruth/gt00' sprintf('%0.4d', i) '.png']);
        GT(:,:,i-1051+1) = Gtimg == 255;
        BG(:,:,i-1051+1) = Gtimg == 50; %Gtimg == 0 | Gtimg == 50 ;
        BckGForeG(:,:,i-1051+1) = Gtimg > 0;
    end
    Im = double(Im);
...
elseif(strcmp(DatabaseName, 'FALL'))
    ...
    for i = 1461:1560
        ...
        RGB(:,:,:,i-1461+1) = imread(['../../Datasets/fall/input/in00' sprintf('%0.4d', i) '.jpg']);
        GTimg = imread(['../../Datasets/fall/groundtruth/gt00' sprintf('%0.4d', i) '.png']);
        GT(:,:,i-1461+1) = GTimg == 255;
        BG(:,:,i-1461+1) = GTimg == 50; %GTimg == 0 | GTimg == 50 ;
        BckGForeG(:,:,i-1461+1) = GTimg > 0;
    end
...
function [ResultsStruct ] = ForegroundExt_Gaussian(Ims, GTIms,BestPhi)
