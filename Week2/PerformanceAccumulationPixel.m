function [pixelTP, pixelFP, pixelFN, pixelTN] = PerformanceAccumulationPixel(pixelCandidates, pixelAnnotation)
    ...
    pixelTP = sum(sum(sum(pixelCandidates>0 & pixelAnnotation>0)));
    pixelFP = sum(sum(sum(pixelCandidates>0 & pixelAnnotation==0)));
    pixelFN = sum(sum(sum(pixelCandidates==0 & pixelAnnotation>0)));
    pixelTN = sum(sum(sum(pixelCandidates==0 & pixelAnnotation==0)));
end
